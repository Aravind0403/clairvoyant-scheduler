"""
collect_alpaca.py — Build training data from Stanford Alpaca (52K)

No login required. Downloads in seconds.
Uses the existing output field — no Ollama needed.
Covers general instruction-following prompts.

Class thresholds (same as all other collectors):
  Short  < 200 tokens  (len // 4)
  Medium 200–799 tokens
  Long   ≥ 800 tokens

Usage:
    python data/pipeline/collect_alpaca.py \\
        [--cap 2000] \\
        [--out data/alpaca_labeled.csv]

Then retrain:
    python model/train.py \\
        --data data/alpaca_labeled.csv \\
        --out  model/predictor_alpaca.json
"""
...
def main(cap: int, out_path: str):
    ...
    print("Downloading Stanford Alpaca (52K, no login required)...")
    ...
    for row in ds:
        ...
        # Alpaca format: instruction + optional input field
        prompt = instruction
        if inp:
    ...
    print(f"\nNext — retrain:")
    print(f"  python model/train.py --data {out_path} --out model/predictor_alpaca.json")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Build training data from Stanford Alpaca 52K (no login needed)"
    )
    parser.add_argument("--cap", type=int, default=2000,
                        help="Max rows per class (default: 2000)")
    parser.add_argument("--out", default="data/alpaca_labeled.csv")
